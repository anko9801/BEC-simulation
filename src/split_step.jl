...
function split_step!(ws::Workspace{N}) where {N}
    ...
    _half_potential_step!(ws, dt / 2, nc, ndim, it)

    apply_kinetic_step!(
        ws.state.psi, ws.state.fft_buf, ws.kinetic_phase,
        ws.fft_plans, nc, ndim,
    )
    ...
    if it && ws.sim_params.normalize_every > 0
        if ws.state.step % ws.sim_params.normalize_every == 0
            _normalize_psi!(ws.state.psi, ws.grid, nc, ndim)
        end
    end
...
function _half_potential_step!(ws::Workspace{N}, dt_half, nc, ndim, imaginary_time) where {N}
    zeeman_diag = collect(zeeman_diagonal(ws.zeeman, ws.spin_matrices.system))

    apply_diagonal_potential_step!(
        ws.state.psi, ws.potential_values, zeeman_diag,
        ws.interactions.c0, dt_half / 2, nc, ndim;
        imaginary_time,
    )
